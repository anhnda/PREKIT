"""
TRIPLE HYBRID MODEL: 
   1. RNN (learns from Temporal only)
   2. Concatenate [RNN_Embed + Last_Values + Static_Features]
   3. XGBoost Classifier

VS BASELINE:
   [Last_Values + Static_Features] -> XGBoost
"""
...
def train_rnn_extractor(model, train_loader, val_loader, criterion, optimizer, epochs=50):
    ...
    # Pre-train using ONLY RNN Features (Since we want RNN to learn TEMPORAL patterns)
    # The Gated Head here acts as a "Proxy XGBoost" to force the RNN to be discriminative
    ...
    for epoch in range(epochs):
        ...
        for t_data, labels, _ in train_loader:  # Ignore static here
