"""
END-TO-END TRAINING (UPDATED): RNN -> [RNN + Last + Static] -> TabPFN (periodically updated)

Key Improvements:
1. STRONGER DISTILLATION: ✅ 100% batches (not 20%), higher weight (0.7 vs 0.5)
2. PERIODIC TABPFN REFIT: ✅ Update TabPFN every 10 epochs with current RNN features
3. Better alignment between proxy and TabPFN throughout training

These changes address the proxy-TabPFN divergence issue observed in TP_EndToEnd.py where
TabPFN AUPR dropped from 0.7530 to 0.6690 during training.
"""
...
# ==============================================================================
# 2. TabPFN Wrapper (Frozen, Differentiable for RNN gradients)
# ==============================================================================
...
# ==============================================================================
# 5. End-to-End Training Function
# ==============================================================================

def train_rnn_end_to_end(rnn_model, train_loader, val_loader, criterion, optimizer, epochs=50):
    """
    Train RNN end-to-end with TabPFN in the loop

    Key difference from previous approaches:
    - NO mimic head
    - TabPFN is part of the training loop
    - RNN learns features that maximize TabPFN's performance
    """

    print("  [Stage 1] Preparing frozen TabPFN on initial features...")

    # First, extract initial features (random RNN + static + last)
    rnn_model.eval()
    init_features = []
    init_labels = []
    ...
    with torch.no_grad():
        for t_data, labels, s_data in train_loader:
            ...
            init_features.append(combined)
            init_labels.extend(labels.numpy())

    init_features = np.vstack(init_features)
    init_labels = np.array(init_labels)

    # Create frozen TabPFN wrapper
    tabpfn_wrapper = DifferentiableTabPFNWrapper(init_features, init_labels).to(DEVICE)
    ...
    for epoch in range(epochs):
        ...
        for t_data, labels, s_data in train_loader:
            ...
            # OPTION 1: STRONGER DISTILLATION
            # Distill from TabPFN on EVERY batch (was 20%)
            # Higher weight for distillation (0.7 vs 0.5)
            tabpfn_preds = tabpfn_wrapper.get_tabpfn_pred(combined)

            # Combined loss: 30% true labels + 70% TabPFN guidance
            label_loss = criterion(proxy_preds, labels)
            distill_loss = F.mse_loss(proxy_preds, tabpfn_preds)
            loss = 0.3 * label_loss + 0.7 * distill_loss
        ...
        # OPTION 2: PERIODIC TABPFN REFIT
        # Refit TabPFN every 10 epochs with current RNN features
        if (epoch+1) % 10 == 0:
            print(f"    [Refitting TabPFN at epoch {epoch+1}]")
            rnn_model.eval()
            refit_features = []
            refit_labels = []
            ...
            with torch.no_grad():
                for t_data, labels, s_data in train_loader:
                    ...
                    refit_features.append(combined.cpu().numpy())
                    refit_labels.extend(labels.numpy())

            refit_features = np.vstack(refit_features)
            refit_labels = np.array(refit_labels)

            # Create new TabPFN wrapper with updated features
            tabpfn_wrapper = DifferentiableTabPFNWrapper(refit_features, refit_labels).to(DEVICE)

            # Re-create optimizer to include new proxy head parameters
            full_optimizer = torch.optim.Adam(
                list(rnn_model.parameters()) + list(tabpfn_wrapper.proxy_head.parameters()),
                lr=0.0005
            )

            print(f"    TabPFN refitted successfully!")
...
def get_triple_features(model, loader):
    """Returns [RNN_Embedding + Last_Value + Static]"""
    model.eval()
    features = []
    labels_out = []
...
def main():
    print("="*80)
    print("END-TO-END TRAINING: RNN trained directly with TabPFN in the loop")
    print("="*80)
    ...
    for fold, (train_full, test_p) in enumerate(trainTestPatients(patients)):
        ...
        print(f"  Fold {fold} Results -> End2End: {metrics_hybrid['auc_pr'][-1]:.3f} vs Baseline: {metrics_base['auc_pr'][-1]:.3f}")
    ...
    ax1.set_title("End-to-End (RNN+TabPFN)")
    ax2.set_title("Baseline (Last+Static)")
    plt.tight_layout()
    plt.savefig("result/end_to_end_tabpfn_vs_baseline.png", dpi=300)
    print("\nPlot saved to result/end_to_end_tabpfn_vs_baseline.png")
    ...
    def print_stat(name, h_metrics, b_metrics):
        ...
        print(f"{name:15s} | End2End: {h_mean:.4f} ± {h_std:.4f}  vs  Baseline: {b_mean:.4f} ± {b_std:.4f}")
