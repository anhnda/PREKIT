...
class ContrastiveProtoHead(nn.Module):
    """
    Advanced TabPFN Mimic with Contrastive Hooks.
    
    1. Soft Multi-Prototypes (3 per class) for stability.
    2. Returns 'raw embeddings' during training to allow 
       Contrastive Loss (pushing away from the wrong class).
    """
    def __init__(self, input_dim, hidden_dim=64, num_prototypes=3, dropout=0.3):
        super(ContrastiveProtoHead, self).__init__()
    ...
    def forward(self, x):
        ...
        # Logits
        logits = dist_1 - dist_0
        
        # --- RETURN EXTRA INFO FOR LOSS FUNCTION ---
        if self.training:
            return torch.sigmoid(logits), emb, p0, p1
        else:
