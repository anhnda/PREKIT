...
- Attention-based RNN Pre-training (mimics TabPFN's transformer nature)
...
# ==============================================================================
# 1. Helpers: Encoder & TabPFN-Mimic Head
# ==============================================================================
...
class TabPFNMimicHead(nn.Module):
    ...
    Key Ideas:
    1. Multi-head self-attention to weight features (like TabPFN's transformer)
    2. Residual connections for stable training
    3. Layer normalization (used in transformers)
    4. Strong regularization to prevent overfitting
    5. Mimics TabPFN's ability to learn feature interactions
    """
    def __init__(self, input_dim, hidden_dim=64, num_heads=4, dropout=0.3):
        ...
        # Multi-head self-attention (key component of transformers/TabPFN)
    ...
    def forward(self, x):
        ...
        # Self-attention (needs 3D input: [batch, seq_len, features])
        # We treat each sample as a sequence of length 1
        h_3d = h.unsqueeze(1)  # [batch, 1, hidden_dim]
...
def train_rnn_extractor(model, train_loader, val_loader, criterion, optimizer, epochs=50):
    ...
    # Pre-train using [RNN + Static] -> TabPFN-Mimic Head
    ...
    print("  [Stage 1] Pre-training RNN with TabPFN-Mimic Head (Attention-based)...")
...
def main():
    ...
    print("Using TabPFN with Attention-Based Pre-training")
    print("="*80)
    ...
    plt.tight_layout()
    plt.savefig("result/triple_hybrid_tabpfn_vs_baseline.png", dpi=300)
    print("\nPlot saved to result/triple_hybrid_tabpfn_vs_baseline.png")
