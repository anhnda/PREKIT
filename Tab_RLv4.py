"""
REINFORCEMENT LEARNING V4: Adaptive Training for Consistent Performance

Goal: Improve worst-case performance (especially Fold 1)

Key improvements over V3:
1. Extended temporal features: Add min, max, slope, count
2. Adaptive RL training: Longer epochs with better early stopping
3. Dynamic learning rate: Adjust based on validation performance
4. Multi-sample evaluation: Average multiple policy rollouts
5. Curriculum learning: Progressive difficulty in reward computation
"""
...
# ==============================================================================
# 5. EXTENDED Feature Extraction (V4: More Statistics)
# ==============================================================================

def extract_extended_features_and_logprobs(policy_net, loader, deterministic=False, temperature=1.0):
    """
    V4: Extract extended features: [Static + Last + Mean + Std + Min + Max + Slope + Count + Z]
    """
...
def train_policy_adaptive_rl(
    ...
    """
    V4: Adaptive RL with dynamic learning rate and longer training
    """

    # Adaptive learning rate based on pretraining quality
    if pretrain_aupr < 0.75:
        lr = 0.0002  # Higher LR for weaker pretraining
    ...
    for epoch in range(epochs):
        ...
        aupr_weight = 0.3 + 0.3 * min(1.0, epoch / 50)  # 0.3 → 0.6
        ...
        if (epoch + 1) % 5 == 0:
            ...
            with torch.no_grad():
                ...
                # Early stopping based on both current and trend
...
def main():
    print("="*80)
    print("RL POLICY V4 (Adaptive + Extended Features) vs BASELINE")
    print("="*80)
    ...
    ax1.set_title("RL Policy V4 + TabPFN Judge")
    ax2.set_title("Baseline (Last + Static)")
    plt.tight_layout()
    plt.savefig("result/tab_rlv4_vs_baseline.png", dpi=300)
    print("\nPlot saved to result/tab_rlv4_vs_baseline.png")
