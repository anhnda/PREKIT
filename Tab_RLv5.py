"""
REINFORCEMENT LEARNING V5: Smart Feature Design

Key Innovation:
- RNN input: Temporal + Handcrafted context (learns what's already captured)
- TabPFN input:
  * Baseline: [Static + Last] (simple baseline)
  * RL:       [Static + Last + Z] (Z must add value beyond just last)

Rationale:
1. RNN sees handcrafted features to understand what's obvious
2. RNN must learn Z that captures NON-obvious patterns
3. No feature explosion to TabPFN
4. Fair comparison: baseline is simple, RL adds only learned Z
"""
...
# ==============================================================================
# 2. Enhanced Dataset (Computes Handcrafted + Last Features)
# ==============================================================================

class HybridDatasetV5(Dataset):
    """
    Computes:
    1. Handcrafted features (for RNN context)
    2. Last values (for TabPFN baseline)
    """
    def __init__(self, patients, feature_names, static_encoder, normalization_stats=None):
        ...
        self.last_values_data = []  # NEW: Store last values separately
        ...
        # Normalize and compute both handcrafted and last features
        for i in range(len(self.data)):
            ...
            # Compute handcrafted features (for RNN context)
            ...
            # Compute last values (for TabPFN)
            last_vals = self._extract_last_values(
                norm_values,
                self.data[i]['masks']
            )
    ...
    def _compute_handcrafted_features(self, norm_values, masks, times):
        """Compute Last + Mean + Std + Min + Max + Slope + Count"""
        handcrafted = []
    ...
    def _extract_last_values(self, norm_values, masks):
        """Extract only last values (simpler)"""
        last_vals = []
...
class RNNPolicyNetworkWithAuxiliary(nn.Module):
    """RNN that uses handcrafted features as context when generating Z"""
...
# ==============================================================================
# 5. Feature Extraction (V5: Only Static + Last + Z to TabPFN)
# ==============================================================================

def extract_simple_features_with_z(policy_net, loader, deterministic=False, temperature=1.0):
    """
    Extract SIMPLE features for TabPFN: [Static + Last + Z]
    RNN still sees handcrafted context, but TabPFN doesn't get them
    """
    ...
    with torch.set_grad_enabled(not deterministic):
        for t_data, labels, s_data, handcrafted, last_vals in loader:
            ...
            # Auxiliary input to RNN (includes handcrafted)
            auxiliary = torch.cat([handcrafted, s_data], dim=1)
            ...
            # Final features to TabPFN: [Static + Last + Z] ONLY
...
# ==============================================================================
# 5B. Baseline: Static + Last Only
# ==============================================================================

def extract_baseline_simple_features(patients_obj, feature_names, encoder, norm_stats):
    """Extract baseline: [Static + Last] (NO Z, NO handcrafted stats)"""
...
def main():
    print("="*80)
    print("RL V5: Smart Feature Design")
    print("RNN sees handcrafted context | TabPFN sees simple features")
    print("="*80)
    ...
    # Calculate auxiliary dimension (for RNN context)
    ...
    print(f"TabPFN feature dimension: {len(FIXED_FEATURES) + len(temporal_feats)} (static + last)")
    ...
    for fold, (train_full, test_p) in enumerate(trainTestPatients(patients, seed=xseed)):
        ...
        # ======================================================================
        # BASELINE: Simple [Static + Last] Only
        # ======================================================================
    ...
    ax1.set_title("RL V5 [Static+Last+Z]")
    ax2.set_title("Baseline [Static+Last]")
    plt.tight_layout()
    plt.savefig("result/tab_rlv5_smart_features.png", dpi=300)
    print("\nPlot saved to result/tab_rlv5_smart_features.png")
    ...
    print("\n" + "="*80)
    print("V5 INNOVATION:")
    print("✓ RNN context: Handcrafted features (learns what's captured)")
    print("✓ TabPFN input: Simple [Static + Last + Z]")
    print("✓ Z must capture non-obvious patterns beyond statistics")
    print("✓ No feature explosion, clean comparison")
    print("="*80)
