"""
REINFORCEMENT LEARNING V4B: Fair Comparison with Extended Baseline

Goal: Test if learned Z adds value beyond comprehensive handcrafted features

Key difference from V4:
- Baseline uses: [Static + Last + Mean + Std + Min + Max + Slope + Count]
- RL uses:       [Static + Last + Mean + Std + Min + Max + Slope + Count + Z]

This is a HARDER test - can the learned representation beat rich statistical features?
"""
...
def extract_extended_features_and_logprobs(policy_net, loader, deterministic=False, temperature=1.0):
    """Extract: [Static + Last + Mean + Std + Min + Max + Slope + Count + Z]"""
...
# ==============================================================================
# 5B. Baseline Extended Feature Extraction (NO RNN, NO Z)
# ==============================================================================

def extract_baseline_extended_features(patients_obj, feature_names, encoder, norm_stats):
    """
    Extract extended features for baseline: [Static + Last + Mean + Std + Min + Max + Slope + Count]
    This matches the RL features but WITHOUT the learned Z
    """
    ...
    for patient in patient_list:
        ...
        for f_idx in range(len(norm_values[0])):
            ...
            if len(valid_idx) > 0:
                ...
                last_vals.append(valid_vals[-1])
                mean_vals.append(np.mean(valid_vals))
                std_vals.append(np.std(valid_vals) if len(valid_vals) > 1 else 0.0)
                min_vals.append(np.min(valid_vals))
                max_vals.append(np.max(valid_vals))
                count_vals.append(len(valid_vals))
        ...
        # Concatenate: [Static + Last + Mean + Std + Min + Max + Slope + Count]
        combined = np.hstack([s_vec, last_vals, mean_vals, std_vals,
                             min_vals, max_vals, slope_vals, count_vals])
...
def train_policy_adaptive_rl(
    ...
    """Adaptive RL with dynamic learning rate"""
...
def main():
    print("="*80)
    print("RL V4B: [Extended Features + Z] vs [Extended Features Only]")
    print("="*80)
    ...
    for fold, (train_full, test_p) in enumerate(trainTestPatients(patients, seed=xseed)):
        ...
        # ======================================================================
        # ENHANCED BASELINE: TabPFN with Extended Features (NO RNN, NO Z)
        # ======================================================================
        print("\n  [Enhanced Baseline] TabPFN with [Static + Last + Mean + Std + Min + Max + Slope + Count]...")

        # Extract extended features for baseline (without RNN/Z)
        ...
        # Train TabPFN on extended features
        ...
        print(f"  Enhanced Baseline Test AUC: {baseline_auc:.4f} | Test AUPR: {baseline_aupr:.4f}")
        print(f"  Fold {fold} Results -> RL: {fold_auc:.3f} vs Enhanced Baseline: {baseline_auc:.3f}")
    ...
    ax1.set_title("RL Policy V4B [Extended + Z]")
    ax2.set_title("Enhanced Baseline [Extended Only]")
    plt.tight_layout()
    plt.savefig("result/tab_rlv4b_vs_enhanced_baseline.png", dpi=300)
    print("\nPlot saved to result/tab_rlv4b_vs_enhanced_baseline.png")
    ...
    def print_stat(name, rl_metrics, base_metrics):
        ...
        print(f"{name:15s} | RL: {rl_mean:.4f} ± {rl_std:.4f}  vs  Enhanced Baseline: {base_mean:.4f} ± {base_std:.4f}  ({improvement:+.2f}%) {symbol}")
    ...
    print("If RL wins: Learned Z captures patterns beyond handcrafted statistics")
    print("If baseline wins: Comprehensive statistics are sufficient, deep learning adds limited value")
    print("="*80)
