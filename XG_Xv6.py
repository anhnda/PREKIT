"""
XG_Xv6: Extended Handcrafted + RL-Optimized Learned Features (XGBoost Version)

SAME AS Tab_Xv6 but using XGBoost instead of TabPFN

Tab_Xv6 PROVEN RESULTS:
- AUC: 0.8797±0.0225 vs Baseline: 0.8608±0.0167 (+2.20%) ✓
- AUPR: 0.8393±0.0296 vs Baseline: 0.8058±0.0364 (+4.15%) ✓

STRATEGY:
1. Use extended handcrafted features [Static + 7 stats per temporal feature]
2. Add RL-optimized learned latent features
   → Use XGBoost DIRECTLY during training (no neural head mismatch)
...
def train_policy_rl(
    ...
    """
    RL training with XGBoost:
    - No pretraining (extended handcrafted features already strong)
    - Direct RL optimization with XGBoost rewards
    """
    print("  [RL Training] Optimizing learned features for XGBoost directly...")
...
def main():
    print("="*80)
    print("XG_Xv6: Extended Handcrafted + RL-Optimized Learned Features (XGBoost)")
    print("="*80)
    print("\nStrategy:")
    print("  1. Extended handcrafted (7 stats/feature)")
    print("  2. RL-optimized learned latent (optimized for XGBoost)")
    ...
    for fold, (train_full, test_p) in enumerate(trainTestPatients(patients, seed=xseed)):
        ...
        print(f"  XG_Xv6 AUC: {fold_auc:.4f} | AUPR: {fold_aupr:.4f}")
    ...
    ax1.set_title("XG_Xv6: Extended Features + RL Learned (XGBoost)")
    ax2.set_title("Baseline (Last Only)")
    plt.tight_layout()
    plt.savefig("result/xg_xv6_vs_baseline.png", dpi=300)
    print("\nPlot saved to result/xg_xv6_vs_baseline.png")
    ...
    def print_stat(name, xv6_metrics, base_metrics):
        ...
        print(f"{name} | XG_Xv6: {xv6_mean:.4f}±{xv6_std:.4f} vs Baseline: {base_mean:.4f}±{base_std:.4f} ({improvement:+.2f}%) {symbol}")

    print_stat("AUC   ", metrics_xv6['auc'], metrics_baseline['auc'])
    print_stat("AUC-PR", metrics_xv6['auc_pr'], metrics_baseline['auc_pr'])

    print("\n" + "="*80)
    print("XG_Xv6 = Tab_Xv6 Strategy with XGBoost:")
    print("✓ Extended handcrafted features")
    print("✓ RL-optimized learned features (optimized for XGBoost)")
    print("✓ No pretraining needed")
    print("="*80)
